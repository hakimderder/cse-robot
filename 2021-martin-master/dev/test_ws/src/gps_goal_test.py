...
class GpsGoal():
  def __init__(self):
    ...
    #rospy.Subscriber('gps_goal_pose', PoseStamped, self.gps_goal_pose_callback)   #   Which topic sends those messages ?
    rospy.Subscriber('gps_goal_pose', PoseStamped, self.gps_goal_pose_callback)
    #rospy.Subscriber('gps_goal_fix', NavSatFix, self.gps_goal_fix_callback)  #  /gps/filtered  or  gps_goal_fix
    rospy.Subscriber('gps/filtered', NavSatFix, self.gps_goal_fix_callback)
...
if __name__ == '__main__':
  ...
  pitch = 0.0
  yaw = 0.0
  if three_args and sys.argv[1][:-1].isdigit() and sys.argv[2][:-1].isdigit():
    #Assign coordinates
    lat = float(sys.argv[1])
    long = float(sys.arv[2])
  else:
    # Coordinates near the start of the pool in level A
    lat = 46.780055
    long = 6.659177
  # Create class
  gpsGoal = GpsGoal()
